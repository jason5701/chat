...
const CreateConversation: FC<CreateConversationProps> = ({ setIsOpened }) => {
  ...
  const toggleHandler = (uid: string) => {
    if (selected.includes(uid)) {
      setSelected(selected.filter((item) => item !== null));
  ...
  return (
    <>
      <div
        ...
        className='fixed top-0 left-0 z-20 flex h-full items-center justify-center bg-[#00000080]'
      >
        <div
          onClick={(e) => e.stopPropagation()}
          className='bg-dark mx-3 w-full max-m-[500px] overflow-hidden rounded-lg'
        >
              ...
              <div className='flex h-96 flex-col items-stretch gap-2 overflow-y-auto py-2'>
                {data?.docs
                  ...
                  .map((doc) => (
                    <div
                      key={doc.data().uid}
                      onClick={() => toggleHandler(doc.data().uid)}
