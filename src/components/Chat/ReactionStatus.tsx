...
import { IMAGE_PROXY, REACTIONS_UI } from '../../constants';

type ReactionsStatusProps = {
  position: 'left' | 'right' | 'left-tab';
  message: MessageItem;
};

const ReactionsStatus = ({ position, message }: ReactionsStatusProps) => {
  const {
    data: usersInfo,
    loading,
    error,
  } = useUserInfo(Object.keys(message.reactions || {}));
...
export default ReactionsStatus;
