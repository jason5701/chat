...
const InputSection = ({
  ...
  const submitHandler = async (e: FormEvent) => {
    ...
    } else if (fileUploading) return;

    if (!inputValue.trim()) return;

    setInputValue('');

    let replacedInpuValue = ` ${inputValue}`;

    Object.entries(EMOJI_REPLACEMENT).map(([key, value]) => {
      value.forEach((item) => {
        replacedInpuValue = replacedInpuValue
          .split(` ${item} `)
          .join(` ${key} `);
      });
    });
    ...
    addDoc(
      collection(db, 'conversations', conversationId as string, 'messages'),
      {
        ...
        content: replacedInpuValue.trim(),
        type: 'text',
  ...
  const uploadFile = async (file: File) => {
    try {
      const TEWNTYY_MB = 1024 * 1024 * 20;

      if (file.size > TEWNTYY_MB) {
      ...
      const fileReference = ref(stroage, formatFileName(file.name));

      await uploadBytes(fileReference, file);

      const downloadURL = await getDownloadURL(fileReference);

      addDoc(collection(db, 'conversations' as string, 'messages'), {
        sender: currentUser?.uid,
        content: downloadURL,
  ...
  const replaceEmojiHandler = (e: any) => {
    if (e.key === ' ') {
  ...
  useEffect(() => {
    ...
    const dropFileHandler = async (e: any) => {
      ...
      for (let i = 0, item; (item = items[i]); i++) {
        ...
        if (entry.isFile) selectedFiles.push(files[i]);
      }
